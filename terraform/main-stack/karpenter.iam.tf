resource "aws_iam_role" "karpenter_controller" {
  name = var.karpenter_controller.iam_role_name
...
resource "aws_iam_policy" "karpenter_controller" {
  name        = var.karpenter_controller.iam_policy_name
  path        = "/"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Effect   = "Allow"
        Action   = "iam:PassRole"
        Resource = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.eks_node_group.iam_role_name}"
      },
      {
        Sid      = "EKSClusterEndpointLookup"
        Effect   = "Allow"
        Action   = "eks:DescribeCluster"
        Resource = "arn:aws:eks:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:cluster/${aws_eks_cluster.this.id}"
      },
      {
